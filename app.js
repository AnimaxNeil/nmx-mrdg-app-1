...
(function () {
  ...
  const COLOR_BG = cssVar('--bg-secondary');
  const COLOR_GRID = cssVar('--bg-card');
  const COLOR_SNAKE_HEAD = cssVar('--accent-secondary');
  const COLOR_SNAKE_BODY = cssVar('--snake-body');
  const COLOR_FOOD = cssVar('--danger');
  const COLOR_FOOD_GLOW = cssVar('--danger-glow');
  const COLOR_WALL = cssVar('--border-wall');
